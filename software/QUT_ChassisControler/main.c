...
int main(void)
{
	...
	if(pedal_read(&brake, &throttle) == 0)throw_error_code(ERROR, ERROR_PEDALS);	
	if(pressure_brake_read(&brakePressureF, &brakePressureR) == 0)throw_error_code(ERROR, ERROR_BRAKE_PRESSURE);
